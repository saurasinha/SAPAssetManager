...
export default class {
    ...
    static setStateVariable(clientAPI, key, value,
        ...
        if ((key === 'StatusStartDate' || key === 'StatusEndDate') && value instanceof Date) {
            clientAPI.nativescript.appSettingsModule.setString(key, value.toJSON());
        } else {
            try {
	...
	*/
    static getStateVariable(clientAPI, key,
        ...
        if (key === 'StatusStartDate' || key === 'StatusEndDate') {
            return new Date(clientAPI.nativescript.appSettingsModule.getString(key));
        } else {
            try {
                let page = clientAPI.evaluateTargetPath('#Page:' + pageName);
                const pageData = page.context.clientData;
     ...
     * @param {*} pageName  - optional. The name of the page where this variable is saved. If not passed in, it defaults to the DefaultStateVariablePage.global which is usually the Overview.page.
     */
    static removeStateVariable(clientAPI, keys,
        ...
        try {
            ...
            } else if (pageData.hasOwnProperty(keys)) {
    ...
    */
    ...
    static unsavedChangesPresent(context) {
        ...
        if (!libThis.isDefined(clientData.controlDefaults) || !libThis.isDefined(valueString)) {
            return true;
        } else {
    ...
    static isInitialSync(contex) {
        return contex.nativescript.appSettingsModule.getBoolean('initialSync', true);
    }

    static setInitialSync(contex) {
        return contex.nativescript.appSettingsModule.setBoolean('initialSync', false);
    }
}
