...
*/
    ...
    static getDeviceId(context) {
        var deviceId = context.evaluateTargetPath('#Application/#ClientData/DeviceId');
    ...
    static  validatePassphrase(context, control) {
        ...
        if (libVal.evalIsEmpty(passphraseText)) {
            // show inline error
            libCom.setInlineControlError(context, control, context.localizeText('passphrase_is_mandatory'));
            return Promise.reject(false);
        }
    ...
    static checkPassphraseLength(context, control) {
        ...
        if (libVal.evalIsEmpty(passphraseText)) {
            ...
            if (passphraseText.length < 4 || passphraseText.length > 6) {
                libCom.setInlineControlError(context, control, context.localizeText('passphrase_length'));
                return Promise.reject(false);
            }
        }
        return Promise.resolve(true);
    ...
    static validateRemark(context, control) {
        ...
        return libThis.readConfiguration(context).then(config => {
            if (config) {
                ...
                if (signatureObject.DigitalSignatureObject_Nav.AllowRemark === 'X' && libVal.evalIsEmpty(remark)) {
                    // show inline error
                    libCom.setInlineControlError(context, control, context.localizeText('remark_is_mandatory'));
                    return Promise.reject(false);
                }
                return Promise.resolve(true);
            } 
        ...
        return libThis.readConfiguration(context).then(config =>{
            if (config) {
                ...
                if (signatureObject.DigitalSignatureObject_Nav.AllowComment === 'X' && libVal.evalIsEmpty(comment)) {
                    // show inline error
                    libCom.setInlineControlError(context, control, context.localizeText('comment_is_mandatory'));
                    return Promise.reject(false);
                }
                return Promise.resolve(true);
            }
